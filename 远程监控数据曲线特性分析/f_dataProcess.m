...
runDistance = (excelData(excelRow,7)-excelData(1,7))/1000; % 行驶距离  m
maxVelocity = max(excelData(:,6));                  %最高车速 km/h
averageVelocity = mean(excelData(:,6)); % 平均车速
maxPitch =max(excelData(:,5));     %最大坡度
averagePitch = mean(excelData(:,5));    %平均坡度
...
%% 求坡度占比
[sortPitch,index] = sort(excelData(:,5));
...
h = plot(distanceData(:,1),excelData(:,3)*100,'r-');
grid on;%%显示网格线
legend('坡度行驶距离曲线');
title('坡度行驶距离曲线');
xlabel('行驶距离');
