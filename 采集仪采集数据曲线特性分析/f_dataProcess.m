...
maxPitch = max (tand(excelData(:,3))*100);     %最大坡度
averagePitch = mean(tand(excelData(:,3))*100);    %平均坡度
% fangcha = var(tand(excelData(:,3))*100); %% 坡度方差
% biaozhuncha = std(tand(excelData(:,3))*100);%% 坡度标准差
% junfanggen = rms(tand(excelData(:,3))*100);%%坡度均方根
% jueduizhidepingjunzhi = mean(abs(tand(excelData(:,3))*100));%%绝对值的平均值
